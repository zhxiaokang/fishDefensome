...
if fish == "cod":
    configfile: "config/rbh_cod_gadMor1.yaml"
elif fish == "medaka":
    configfile: "config/rbh_mdk.yaml"
elif fish == "stickleback":
    configfile: "config/rbh_stkb.yaml"
elif fish == "killifish":
    configfile: "config/rbh_fdls.yaml"

# The end rule for the whole workflow: what's the final output you want from this workflow?
rule end:
    input:
        fish_zf_filter = config["OUTPUTPATH"] + "/pid_gid_gname_fish_zf_filter.tsv"
...
rule combine:
    ...
    shell:
        """
        cat {input.rbh} {input.best_hit_miss} | awk -F'[\t.]' '{{print $1"\t"$3}}' > {output.bests}
        """

# retrieve gene ID and gene name of protein id from ENSEMBL
rule retrieve:
    input:
        bests = config["OUTPUTPATH"] + "/bests_fish_zf.txt"
    output:
        fish_table = config["OUTPUTPATH"] + "/pid_gid_gname_fish.tsv",
        zf_table = config["OUTPUTPATH"] + "/pid_gid_gname_zf.tsv",
        fish_zf_table = config["OUTPUTPATH"] + "/pid_gid_gname_fish_zf.tsv"
    params:
        fish = fish
    shell:
        """
        Rscript scripts/pid2gid_gname_fish.R {input} {params.fish} {output.fish_table} {output.zf_table}
        awk 'ARGIND==1 {{fish[$1]=$2"\t"tolower($3)}} ARGIND==2 {{zf[$1]=$2"\t"tolower($3)}} ARGIND==3 {{print $1"\t"fish[$1]"\t"$2"\t"zf[$2]}}' {output.fish_table} {output.zf_table} {input} > {output.fish_zf_table}
        """

# pick the zebrafish genes whose names start with the names in Marta's defensome gene list (only look at the last column!!! not the 3rd col which is the gene name of fish)
rule filter:
    input:
        fish_zf_table = config["OUTPUTPATH"] + "/pid_gid_gname_fish_zf.tsv",
